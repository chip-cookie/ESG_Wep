        ...
        return (
            <div className="bg-white/95 backdrop-blur-md border border-slate-100 p-4 rounded-xl shadow-2xl text-xs z-50">
                ...
                {payload.map((entry: any, index: number) => {
                    const isCurrency = ['K-ETS', 'EU-ETS', 'UK-ETS', '한국', '유럽', 'Cost', 'Price', 'Revenue', 'Capex'].some(key => entry.name?.includes(key));
                    const currencySymbol = (entry.name?.includes('EU') || entry.name?.includes('유럽')) ? '€' : (entry.name?.includes('UK') ? '£' : '₩');
